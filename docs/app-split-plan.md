...
> 작성: 2026-09-18 · 상태: **1·2단계(스크립트·스타일·데이터 분리) 완료 · 프리렌더 경량화 1단계 완료 · 3단계(로드 전략)에서 폰트 적용 완료, CLS 개선이 다음 후보**
...
- ✅ **본문 서체(Pretendard, 외부 CDN)를 렌더 차단에서 뻗습니다** (2026-09-18).
  `rel="stylesheet"` 로 두면 홈·정적 페이지 첫 페인트가 이 30KB(그리고 뒤따르는 woff2 574KB)를
  기다렸습니다. 이제 `rel="preload" as="style"` + `onload` 로 받아 두고 도착하면 스타일로 바꿉니다
  (도착 전에는 시스템 글꼴로 먼저 그립니다 — FOUT). 자바스크립트를 끈 사용자는 `<noscript>` 의
  stylesheet 을 그대로 쓰므로 예전과 같습니다(`index.html` · `build-pages.mjs` 의 페이지·404 3곳).
  실측(`npm run perf`, 같은 조건 3회씩): **FCP·LCP 528/548/516ms → 488/444/456ms · load 약 1.5s → 약 1.0s**,
  CLS 는 늘지 않았습니다. 이득이 큰 편은 아니지만(첫 페인트 −60ms 남짓) 비용이 없고 되돌리기도 쉬워 적용했습니다.
