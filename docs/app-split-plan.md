...
> 작성: 2026-09-18 · 상태: **1·2단계(스크립트·스타일·데이터 분리) 완료 · 프리렌더 경량화 1·2단계 완료 ·
> 3단계(로드 전략)에서 폰트 자체 서브셋·CLS 해결 완료, 첫 화면에 필요 없는 데이터 미루기가 남음**
...
### 3단계 — 로드 전략 다듬기 (✅ 폰트 완료 · 데이터 분리는 2단계-b)

- ✅ **본문 서체를 렌더 차단에서 뻗고, 자체 서브셋으로 바꿨습니다** (2026-09-18).
  1차: `rel="stylesheet"` 로 두면 첫 페인트가 CDN CSS 30KB(그리고 뒤따르는 woff2) 를 기다렸으므로
  `rel="preload" as="style"` + `onload` 로 받아 두고 도착하면 스타일로 바꿨습니다(FOUT).
  2차(17차 점검): 그 CDN 이 홈 첫 방문에 **조각 39개 · 574KB** 를 받는다는 것을 실측으로 확인하고,
  사이트에 실제로 나오는 글자만 담은 서브셋을 만들어 자체 호스팅으로 바꿨습니다 —
  **폰트 574KB → 195KB(파일 1개) · 첫 방문 1,016.3KB → 628.6KB · 요청 70 → 50개 · CLS 0.05 → 0.00**.
  만드는 도구는 `tools/make-font-subset.py`(+`tools/font-charset.mjs`), 감사는 `audit:site` 5-3,
  브라우저 점검은 홈·정적 페이지의 실제 글꼴 적용 확인입니다(`docs/ux-review.md` §22-1).
- ✅ **CLS 0.51 → 0.05 해결** (2026-09-18 · `npm run perf` 3회 기준 0.048~0.055).
  shift 출처를 임시로 찍어 보니 화면이 밀린 곳은 폰트 교체가 아니라 **세 가지가 뒤질게 그려지는 지점**이었습니다:
  ① 홈이 아닌 화면용 검색창을 로드 뒤에 숨기며 내용이 140px 위로 밀림, ② 넓은 화면에서 전체 목록(53섹션)을
  로드 뒤에 펼치며 150px 넘게 밀림, ③ 응원 문구를 로드 뒤에 채우며 한 줄(40px) 밀림.
  셋 다 **첫 화면(HTML·CSS)에서 이미 최종 모습이 되도록** 고쳤습니다 — 검색창은 처음부터 `display:none`,
  목록 펼침은 CSS 미디어 쿼리가 결정(JS 는 반대 상태만 클래스로 붙임), 응원 문구 자리는 `min-height` 로 확보.
  거기에 상단바가 글꼴 너비에 따라 1줄 ↔ 2줄로 갈리며 47px 밀리던 문제도 함께 잡았습니다
  (`progress-wrap` 의 `min-width` 160 → 120px 로 줄바꿈 경계에 걸리지 않게).
- 첫 화면에 필요 없는 데이터(`data/extra.js`)와 무거운 섹션 렌더를 더 미루는 일은 미완입니다.
- 프리렌더(본문 마크업) 경량화는 `docs/prerender-split-plan.md` 에서 따로 다뤘습니다 — 1단계
  (`freqGrid` → `units/frequency.html`)를 실행해 `index.html` 이 239.2KB → **196.1KB**(문자 기준)가 되었습니다.
...
- `index.html` ≤ 120KB(raw), 인라인 `<style>`·`<script>` 는 첫 페인트용 부트스트랩(테마·등록)만 남김.
  ...
  `index.html` 283.5KB(당시 기준).
  이후 프리렌더 경량화 1단계까지 반영한 현재 값은 **196.1KB(문자) · 235.6KB(디스크 바이트)** 이고,
  남은 121.7KB(62.1%)는 **프리렌더 마크업**(검색·JS 없이 읽히는 본문)이라 파일로 뺄 수 없습니다.
  자세한 판단은 `docs/prerender-split-plan.md` 참고.
