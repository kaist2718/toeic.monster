...
- ⚠️ **남은 문제(기록만)**: 첫 방문 **CLS 0.51**(재방문 0.28)은 기준(0.1)을 크게 넘습니다.
  `tools/measure-perf.mjs` 에 일시적으로 shift 출처를 찍어 보니, 웹폰트가 도착해 글꼴이 바뀌는 순간
  (871ms·1,309ms) `main` 이 위로 밀리는 두 번의 큰 이동이었습니다 — 즉 **폰트 교체(FOUT)가 원인**입니다.
  지금 그대로 두는 이유: ①폰트를 막으면 첫 페인트가 다시 느려지고, ②`size-adjust`/`ascent-override` 로
  대체 글꼴 메트릭을 맞추는 작업은 효과 검증에 시간이 더 필요합니다. 다음에 다룰 후보로 남깁니다.
