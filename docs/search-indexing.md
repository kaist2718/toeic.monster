...
과 단위 검색 유입과 공유를 위한 선택이고, 책 페이지 링크 목록(「과 하나씩 따로 보기」)·같은 책의 다른 과 목록·사이트맵으로 발견됩니다.

문법 교재의 과 페이지에는 `✏️ 앱에서 이 과 문제 풀기` → `../index.html?level=basic&ch=5#grammar-quiz` 링크가 있어,
검색으로 들어온 사람이 **그 과의 연습 문제를 바로** 풀 수 있습니다(색인과는 무관한 앱 안에서의 동선).

> 감사(`npm run audit:site`)가 canonical 이 배포 주소와 정확히 같은지, `<title>`·canonical 이 다른 페이지와 겹치지 않는지,
> 과 페이지에 `position`·`isPartOf` 구조화 데이터가 있는지를 검사합니다 — 주소가 어긋나면 중복 색인의 원인이 됩니다.
